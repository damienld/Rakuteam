...
# app3.py
def app():
    st.title("Projet d'analyse ")
    st.write("La mise en catalogue de listes de produits via la catégorisation des titres et des images, est un problème fondamental pour toute place de marché électronique, avec des applications allant de la recherche et recommandations personnalisées, à la compréhension des requêtes. ")
    st.write("Dans ce contexte, le déploiement d'approches de machine learning multimodales, exploitant l’analyse conjointe des données image et texte associées aux articles, est une technique utile pour les entreprises de commerce électronique car cela permet de gagner en efficacité et précision.")
    st.write("Dans le cadre d’un challenge organisé par l'ENS et de notre formation data scientist au sein de DataScientest, nous avons pu travailler sur la classification de produits à grande échelle, en développant un projet visant à prédire le type de chaque produit tel que défini dans le catalogue de Rakuten France.")
    st.write("Sachant que les progrès dans ce domaine de recherche ont été limités en raison du manque de données disponibles, le fait que la société Rakuten propose un set de données exploitable a constitué une vraie opportunité.")
    ...
    st.write("Pour résumer, nos objectifs lors de ce projet étaient:"
                "\n\n"
                ...
                "\n\n"
                "•	proposer une application permettant de classer en direct tout nouvel article proposé")
